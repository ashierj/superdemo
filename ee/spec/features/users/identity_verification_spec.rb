...
RSpec.describe 'Identity Verification', :clean_gitlab_redis_rate_limiting, :js,
  ...
  describe 'signing up' do
    ...
    it 'shows the verification page' do
      expect(page).to have_content("For added security, you'll need to verify your identity in a few quick steps.")

      expect(page).to have_content(
        format("We've sent a verification code to %{email}", email: obfuscated_email(new_user.email))
      )
    end
    ...
    describe 'verifying the code' do
      ...
      it 'shows client side empty eror message' do
        ...
        expect(page).to have_content('Enter a code.')
      end

      it 'shows client side invalid error message' do
        verify_code 'xxx'

        expect(page).to have_content('Enter a valid code.')
      end
      ...
      it 'shows a server side rate limited error message' do
        ...
        expect(page).to have_content(
          format(
            "You've reached the maximum amount of tries. Wait %{interval} or send a new code and try again.",
            interval: '10 minutes'
          )
        )
      end
    ...
    describe 'resending the code' do
      it 'rate limits resending', quarantine: 'https://gitlab.com/gitlab-org/gitlab/-/issues/375306' do
        ...
        expect(page).to have_content(
          format(
            "You've reached the maximum amount of resends. Wait %{interval} and try again.",
            interval: 'about 1 hour'
          )
        )
      end
      ...
      it 'resends a different code' do
        ...
        perform_enqueued_jobs do
          click_link 'Send a new code'
          expect(page).to have_content('A new code has been sent.')
        end
