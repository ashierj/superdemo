...
RSpec.describe Repository, :elastic, feature_category: :global_search do
  ...
  describe 'searching' do
    it 'searches blobs' do
      expect(project.repository.elastic_search('def popen', type: 'blob')[:blobs][:total_count]).to eq(1)
      expect(project.repository.elastic_search('files/ruby/popen.rb', type: 'blob')[:blobs][:total_count]).to eq(1)
      expect(project.repository.elastic_search('def | popen', type: 'blob')[:blobs][:total_count] > 1).to be_truthy
    end

    it 'searches commits' do
      expect(project.repository.elastic_search('initial', type: 'commit')[:commits][:total_count]).to eq(1)

      root_ref = project.repository.root_ref_sha.upcase
      expect(project.repository.elastic_search(root_ref, type: 'commit')[:commits][:total_count]).to eq(1)

      partial_ref = root_ref[0...5]
      expect(project.repository.elastic_search(partial_ref, type: 'commit')[:commits][:total_count]).to eq(1)
      expect(project.repository.elastic_search(partial_ref + '*', type: 'commit')[:commits][:total_count]).to eq(1)
    end
  end
  ...
  context 'filtering' do
    it 'can filter blobs' do
      ...
      expect(project.repository.elastic_search('def | popen filename:test', type: 'blob')[:blobs][:total_count]).to eq(1)

      # Should not find anything, since filename doesn't match on path
      expect(project.repository.elastic_search('def | popen filename:files', type: 'blob')[:blobs][:total_count]).to eq(0)

      # Finds files/ruby/popen.rb, files/markdown/ruby-style-guide.md, files/ruby/regex.rb, files/ruby/version_info.rb
      expect(project.repository.elastic_search('def | popen path:ruby', type: 'blob')[:blobs][:total_count]).to eq(4)

      # Finds files/markdown/ruby-style-guide.md
      expect(project.repository.elastic_search('def | popen extension:md', type: 'blob')[:blobs][:total_count]).to eq(1)

      # Finds files/ruby/popen.rb
      expect(project.repository.elastic_search('* blob:7e3e39ebb9b2bf433b4ad17313770fbe4051649c', type: 'blob')[:blobs][:total_count]).to eq(1)

      # filename filter without search term
      count = project.repository.ls_files('master').count { |path| path.split('/')[-1].include?('test') }
      expect(project.repository.elastic_search('filename:test', type: 'blob')[:blobs][:total_count]).to eq(count)
      expect(project.repository.elastic_search('filename:test', type: 'blob')[:blobs][:total_count]).to be > 0

      # extension filter without search term
      count = project.repository.ls_files('master').count { |path| path.split('/')[-1].split('.')[-1].include?('md') }
      expect(project.repository.elastic_search('extension:md', type: 'blob')[:blobs][:total_count]).to eq(count)
      expect(project.repository.elastic_search('extension:md', type: 'blob')[:blobs][:total_count]).to be > 0

      # path filter without search term
      count = project.repository.ls_files('master').count { |path| path.include?('ruby') }
      expect(project.repository.elastic_search('path:ruby', type: 'blob')[:blobs][:total_count]).to eq(count)
      expect(project.repository.elastic_search('path:ruby', type: 'blob')[:blobs][:total_count]).to be > 0
