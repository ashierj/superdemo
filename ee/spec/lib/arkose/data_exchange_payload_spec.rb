...
RSpec.describe Arkose::DataExchangePayload, feature_category: :instance_resiliency do
  ...
  describe '#build' do
    ...
    it 'can be decrypted with the right key' do
      freeze_time do
        encoded_initialization_vector, encoded_encrypted_text_and_tag = result.split('.')
        ...
        decipher.auth_data = ""
        decrypted = decipher.update(encrypted_text) + decipher.final # rubocop:disable Rails/SaveBang -- Not an ActiveRecord::Base#update call

        expect(Gitlab::Json.parse(decrypted)).to include(json_data)
      end
    end
