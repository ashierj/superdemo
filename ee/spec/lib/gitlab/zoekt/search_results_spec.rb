...
RSpec.describe ::Gitlab::Zoekt::SearchResults, :zoekt, feature_category: :global_search do
  ...
  describe 'blobs' do
    ...
    it 'finds blobs from searched projects only' do
      ...
      results = described_class.new(user, 'project_name_regex', [project_1.id], node_id: node_id)
      ...
      results = described_class.new(user, 'project_name_regex', [project_1.id, project_3.id], node_id: node_id)
      result_project_ids = results.objects('blobs').map(&:project_id)
